...
def init(n, precomputed=True, dataset=None, topper=None, pref_matrix=None):
    """
    ...
    if(precomputed and topper == None and dataset == None):
        if(n == 10):
            scores = np.array([8.2, 100.,  48.174,  42.926,  52.671,
                               64.874,  27.607, 62.456,  64.792,  26.614])
        elif(n == 25):
    ...
    elif pref_matrix is not None:
        scores = np.loadtxt(pref_matrix)

        copeland_score = (scores > 0.5).sum(
            axis=1) / float(len(scores) - 1)
        # true_top = np.argmax(copeland_score)
        true_top = np.argwhere(
            copeland_score == np.amax(copeland_score)).flatten()
...
def get_args():
    ...
    parser.add_argument("--n", default=10, type=int,
                        help="number of items N in the dataset")
    parser.add_argument("--experiments", default=10, type=int,
                        help="number of experiments to average the metrics over")
    parser.add_argument("--iterations", default=100, type=int,
                        help="number of iterations the algorithm runs for in a single experiment")
    parser.add_argument("--budget", default=10, type=int,
                        help="the multiple of N you want your max budget to be, metrics will be evaluated in steps of N")
    parser.add_argument("--precomputed", action='store_true',
                        help="use precomputed score vectors for the synthetic case")
    parser.add_argument("--dataset", default=None,
                        help="real-world data you want to evaluate: sushi-A, sushi-B, jester, netflix, movielens")
    parser.add_argument("--toppers", default=None, nargs='+', type=int,
                        help="array of x when the topper has score x and the rest 100-x")
    parser.add_argument("--compute", default="exact",
                        help="The compute method to evaluate pairs for active picking")
    parser.add_argument("--name", default="",
                        help="The log file name to save the csv data")
    parser.add_argument("--save_dir", default=".",
                        help="The save directory of the log file")
    parser.add_argument("--no_save", action="store_true",
                        help="Do not save log file")
    parser.add_argument("--pref_matrix", default=None,
                        help="The pref_matrix file")
