...
def SELECT(n, m, scores, data, comp_matrix):
    ...
    total_votes = 0
    while(np.size(a) > 1):
        a, num_votes, data, comp_matrix = halve(
            m, a, scores, data, comp_matrix)
...
def reset(n, scores, initialization="heuristic"):
    ...
    if(initialization == "heuristic"):
        ...
        top, initial, data, comp_matrix = SELECT(
            n, 1, scores, data, comp_matrix)
...
def IMC_prune(n, comp_matrix, pair, phi, MC, MC_sq, metric="L2"):
    ...
    if((comp_matrix[j][i]+comp_matrix[i][j]) == 0):
        delta_i = - ((comp_matrix[j][i]) /
                     (comp_matrix[j][i]+comp_matrix[i][j]+1))/n
    else:
        delta_i = - ((comp_matrix[j][i])/(comp_matrix[j][i]+comp_matrix[i][j]+1))/n + (
            (comp_matrix[j][i])/(comp_matrix[j][i]+comp_matrix[i][j]))/n
    ...
    if((comp_matrix[j][i]+comp_matrix[i][j]) == 0):
        delta_j = - ((comp_matrix[i][j]+1) /
                     (comp_matrix[j][i]+comp_matrix[i][j]+1))/n
    else:
        delta_j = - ((comp_matrix[i][j]+1)/(comp_matrix[j][i]+comp_matrix[i][j]+1))/n + (
            (comp_matrix[i][j])/(comp_matrix[j][i]+comp_matrix[i][j]))/n
    new_error = (pi[j]/(1+(-delta_j*new_MC_j[j])+(delta_j*new_MC_i[j]))
                 ) * ((-delta_j*new_MC_j) + (delta_j*new_MC_i))
    new_pi = pi - new_error
...
def IMC_update(n, comp_matrix, pair, inc, phi, chain, MC, est, metric="L2"):
    ...
    IP_hash = MC + np.ones(n+1)[:, np.newaxis]*np.dot(error,
                                                      temp) - (MC[:, i][:, np.newaxis]*error)/phi[i]
    pi = normalize(pi)
...
def pick_a_pair_old(n, comp_matrix, estimates, top, chain, A_hash, ranking, k=1, method="weighted"):
    ...
    for p in range(1, k+1):
        for q in range(p+1, n+1):
            ...
            j = ranking[n-q]
            # IMC for (i, j)
            m1, pi, new_chain, IP_hash = IMC_update(
                n, comp_matrix, (i, j), True, estimates, chain, A_hash, estimates)
            m1, pi, new_chain, IP_hash = IMC_update(
                n, comp_matrix, (j, i), False, pi, new_chain, IP_hash, estimates)
            # IMC for (j, i)
            m2, pi, new_chain, IP_hash = IMC_update(
                n, comp_matrix, (j, i), True, estimates, chain, A_hash, estimates)
            m2, pi, new_chain, IP_hash = IMC_update(
                n, comp_matrix, (i, j), False, pi, new_chain, IP_hash, estimates)
            # Update Max
            if(method == "average"):
                # Average
                m = m1+m2
                ...
                # Weighted
                m = (estimates[i]*m1 + estimates[j]*m2) / \
...
def pick_a_pair(n, comp_matrix, estimates, chain, A_hash, ranking, ranks, top, compute="exact", method="weighted"):
    ...
    for p in range(1, n+1):
        if(not (p == top)):
            ...
            if(compute == "exact"):
                m1 = IMC_prune(n, comp_matrix, (i, j),
                               estimates, A_hash, A_hash_sq)
                m2 = IMC_prune(n, comp_matrix, (j, i),
                               estimates, A_hash, A_hash_sq)
    ...
    for tp, topper in tqdm.tqdm(enumerate(toppers), desc="toppers"):
        ...
        for exp in tqdm.tqdm(range(experiments), desc="experiments"):
            for itr in tqdm.tqdm(range(iterations), desc="iterations"):
                data, initial, comp_matrix, chain, A_hash, estimates = reset(
                    n, scores)
                ...
                for batch in range(int(budget/(n))):
                    for i in range(initial, aux_bgt):
                        ...
                        (p, q) = pick_a_pair(n, comp_matrix, estimates,
                                             chain, A_hash, ranking, ranks, top, compute=compute)
                        if(Vote(scores[p-1], scores[q-1])):
                            data.append((p, q))
                            # comp_matrix[p][q] += 1
                            m, pi, new_chain, IP_hash = IMC_update(
                                n, comp_matrix, (p, q), True, estimates, chain, A_hash, estimates)
                            m, pi, new_chain, IP_hash = IMC_update(
                                n, comp_matrix, (q, p), False, pi, new_chain, IP_hash, estimates)
                            comp_matrix[p][q] += 1
                        else:
                            data.append((q, p))
                            # comp_matrix[q][p] += 1
                            m, pi, new_chain, IP_hash = IMC_update(
                                n, comp_matrix, (q, p), True, estimates, chain, A_hash, estimates)
                            m, pi, new_chain, IP_hash = IMC_update(
                                n, comp_matrix, (p, q), False, pi, new_chain, IP_hash, estimates)
                            comp_matrix[q][p] += 1
    ...
    for exp in tqdm.tqdm(range(experiments), desc="experiments"):
        for itr in tqdm.tqdm(range(iterations), desc="iterations"):
            ...
            if((top-1) in true_top):
                correctPred += 1
            print(
                f"\nitr:{itr}, predTop:{top-1}, total_comparison:{total_comparison}")
...
if __name__ == "__main__":
    ...
    if args.dataset is not None:
        ...
        print(
            f"Running {Experiments} experiments of {Iterations} iterations each\n on the \"{args.dataset}\" dataset where N = {N}.\n")
        ...
        Ranking, Ranks, Data, Scores, True_top, Estimates, RC, PF = run_simulation_custom(
            N, Toppers, Experiments, Iterations, Budget, RC, PF, compute=args.compute)
        ...
        if args.precomputed:
            print(
                "This score vector has been precomputed and stored for reproducibility.\n")
